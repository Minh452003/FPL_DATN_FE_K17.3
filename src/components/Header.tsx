...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (carts?.data?.products) {
      const totalQuantity = carts.data.products.reduce(
  ...
  const logout = () => {
    ...
    localStorage.removeItem("accessToken").then(()=>{
      navigate("/");
      setLogout(true);
    })
  };
                  ...
                  <div className="ml-3 items-center flex relative group">
                    <Link to={"/carts"}>
                      <div className="relative">
                        ...
                        />
                        {cartItemCount > 0 && (
                          <div className="absolute top-1 right-0 bg-orange-500 rounded-full h-5 w-5 text-white text-center">
                            {cartItemCount}
                          </div>
                        )}
                        {cartItemCount === 0 && (
